...
export default function GeneratePage() {
  ...
  const previewArtifact =
    artifacts.find((a) => String(a.id) === selectedArtifactId) ?? null;
  ...
  function regenerateFromSelectedProject() {
    ...
    const linkedJob = parsedJobs.find(
  ...
  async function fetchArtifacts(projectId: string) {
    ...
    setArtifacts(data);
    const best =
      data.find((a) => a.artifact_type === "final_pdf") ??
      data.find((a) => a.mime_type === "application/pdf");
  ...
  async function startGenerate() {
    ...
    if (!model || !apiKeyEnv || (isCustomCompat && !apiBase)) return;
    ...
    if (!res.ok) {
      ...
      const message = normalizeApiError(res.status, text);
    ...
    pollRef.current = setInterval(async () => {
      try {
        const [statusRes, logsRes] = await Promise.all([
          ...
          fetch(
            `${API}/users/${selectedUid}/projects/${projectId}/logs?limit=300`,
          ),
        ]);
  ...
  async function stopRun() {
    ...
    await fetch(
      `${API}/users/${selectedUid}/projects/${activeProjectId}/stop`,
      { method: "POST" },
    );
  }

  async function deleteProject(projectId: string) {
    await fetch(`${API}/users/${selectedUid}/projects/${projectId}`, {
      method: "DELETE",
    });
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto" }}>
            <div style={{ padding: "8px 12px" }}>
              ...
              <input
                type="text"
                placeholder="e.g. Google – Senior SWE"
                value={jobLabel}
          ...
          <div
            style={{ borderTop: "1px solid var(--line)", padding: "8px 12px" }}
          >
            <div className="bp-label" style={{ marginBottom: 4 }}>
              RESUME TEMPLATE
            </div>
          ...
          >
            <span className="bp-label" style={{ marginRight: 4 }}>
              SECTIONS
            </span>
          ...
          >
            <div style={{ display: "flex", gap: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <select
                  value={selectedParsedJobId}
                  onChange={handleParsedJobChange}
                  style={selectStyle}
                >
            ...
            {mandatoryWords.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                <span className="bp-label" style={{ marginTop: 4 }}>
                  REQUIRED:
                </span>
            ...
            {suggestedWords.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                ...
                {suggestedWords
                  .filter((w) => !mandatoryWords.includes(w))
                  .map((w) => (
                    <button
                      key={w}
                      onClick={() => addMandatoryWord(w)}
                      style={{
          ...
          >
            {logs.length === 0 && !isRunning && (
              <span style={{ color: "var(--muted)" }}>
                &gt; Start a generation run to see live output.
              </span>
            )}
            {logs.map((l, i) => (
              <div key={i} className={`log-${l.level}`}>
                <span style={{ color: "var(--muted)" }}>
                  {new Date(l.ts * 1000).toLocaleTimeString("en-GB")}
                </span>{" "}
                {l.text}
              </div>
            ))}
            ...
            <>
              ...
              >
                ...
                <a
                  ...
                  download={downloadNameForProject(
                    selectedProject?.name ?? jobLabel,
                    previewArtifact,
                  )}
                  style={{
                    marginLeft: "auto",
...
function downloadNameForProject(
  projectName: string,
  artifact: Artifact,
): string {
...
function MarkdownViewer({ path }: { path: string }) {
  ...
  useEffect(() => {
    fetch(
      `/api/artifacts/download?path=${encodeURIComponent(path)}&disposition=inline`,
    )
      .then((r) => r.text())
      .then(setContent)
