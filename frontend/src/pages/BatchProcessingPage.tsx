...
function normalizeApiError(status: number, text: string): string {
  ...
  if (isHtml) {
    return `HTTP ${status}. Received HTML instead of API JSON. Check frontend proxy / backend server routing.`;
  }
...
const MODEL_PRESETS: Record<string, ModelPreset> = {
  ...
  "GPT OSS 120B": { model: "groq/qwen/qwen3-32b", key_env: "GROQ_API_KEY" },
...
export default function BatchProcessingPage() {
  ...
  const [customKeyEnv, setCustomKeyEnv] = useState("OPENAI_API_KEY");
  const [customApiBase, setCustomApiBase] = useState("");
  const [templates, setTemplates] = useState<ResumeTemplate[]>([]);
  const [selectedTemplateId, setSelectedTemplateId] = useState<string>("");
  const [omissions, setOmissions] = useState<Omissions>({
    ...DEFAULT_OMISSIONS,
  });
  ...
  useEffect(() => {
    ...
    return () => {
      _batchListeners.delete(rerender);
    };
  }, [rerender]);
  ...
  function buildMandatoryWords(job: ScrapedJob): string[] {
  ...
  function resolveModelConfig() {
    ...
    const isCustomCompat = Boolean(preset?.custom_mode);
    const model = isCustomCompat ? customModel.trim() : preset.model;
    const apiKeyEnv = isCustomCompat
      ? customKeyEnv.trim() || preset.key_env
      : preset.key_env;
    const apiBase = isCustomCompat ? customApiBase.trim() : "";
    return { preset, isCustomCompat, model, apiKeyEnv, apiBase };
  }
  ...
  function toggleAll() {
    if (_batchState.isProcessing) return;
    const next =
      _batchState.selectedJobIds.size === jobs.length
        ? new Set<string>()
  ...
  async function startBatchProcessing() {
    if (
      !selectedUid ||
      _batchState.selectedJobIds.size === 0 ||
      _batchState.isProcessing
    )
      return;

    const { model, apiKeyEnv, apiBase, isCustomCompat } = resolveModelConfig();
    if (!model || !apiKeyEnv || (isCustomCompat && !apiBase)) {
      appendBatchLog(
        "ERROR: Custom provider requires model, API key env, and API base URL.",
      );
      return;
    }

    const jobsToProcess = jobs.filter((j) =>
      _batchState.selectedJobIds.has(j.id),
    );
    ...
    appendBatchLog(
      `Starting batch for ${jobsToProcess.length} job(s) using ${selectedPreset}...`,
    );
    appendBatchLog(`Model: ${model}`);
    if (apiBase) appendBatchLog(`API Base: ${apiBase}`);
    appendBatchLog(`Template: ${selectedTemplateName}`);

    for (let i = 0; i < jobsToProcess.length; i++) {
      const job = jobsToProcess[i];
      setBatchState({
        progress: { current: i + 1, total: jobsToProcess.length },
      });
      appendBatchLog(`---`);
      appendBatchLog(
        `[${i + 1}/${jobsToProcess.length}] ${job.title} @ ${job.company}`,
      );

      try {
        const mandatoryWords = buildMandatoryWords(job);
        appendBatchLog(`Mandatory words seeded: ${mandatoryWords.length}`);
        ...
        if (!genRes.ok) {
          ...
          appendBatchLog(
            `ERROR: Failed to start generation: ${normalizeApiError(genRes.status, text)}`,
          );
          continue;
        }
        ...
        while (!done) {
          ...
          try {
            const [sRes, lRes] = await Promise.all([
              ...
              fetch(
                `${API}/users/${selectedUid}/projects/${projectId}/logs?limit=500`,
              ),
            ]);

            if (lRes.ok) {
              const logs: { ts: number; text: string; level: string }[] =
                await lRes.json();
            ...
            if (sRes.ok) {
              ...
              if (["completed", "failed", "stopped"].includes(s.state)) {
                ...
                if (s.state === "failed" && s.error_message) {
                  appendBatchLog(`ERROR: ${String(s.error_message)}`);
                }
              }
            }
          } catch {
  ...
  const { preset, isCustomCompat, model, apiKeyEnv, apiBase } =
    resolveModelConfig();
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        >
          ...
          >
            ...
            {/* Row 1: Profile, Model, Template */}
            <div style={{ display: "flex", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div className="bp-label" style={{ marginBottom: 4 }}>
                  PROFILE
                </div>
                <select
                  value={selectedUid}
              ...
              <div style={{ flex: 1.5 }}>
                ...
                >
                  {templates.length === 0 ? (
                    <option value="">Default runtime template</option>
                  ) : (
                    templates.map((t) => (
        ...
        >
          ...
          >
            ...
            {isProcessing && (
              <>
                ...
                <span style={{ fontSize: 10, color: "var(--cyan-bright)" }}>
                  RUNNING
                </span>
              </>
            )}
          ...
          >
            {logs.length === 0 ? (
              <span style={{ color: "var(--muted)" }}>
                &gt; Select jobs and click Start Batch to begin.
              </span>
            ) : (
              logs.map((log, i) => (
                <div
